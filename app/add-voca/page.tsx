...
export default function AddVocaPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      for (const item of readyItems) {
        ...
        console.log("Uploading voca payload:", {
          coursePath: payload.coursePath,
          dayName: payload.dayName,
          wordCount: payload.words.length,
          wordsPreview: payload.words.slice(0, 3),
        });
  ...
  return (
    <PageLayout>
      ...
      {tabIndex === 0 && (
        <CsvUploadTab items={csvItems} onItemsChange={setCsvItems} />
      )}
      {tabIndex === 1 && (
        <UrlUploadTab items={urlItems} onItemsChange={setUrlItems} />
      )}
